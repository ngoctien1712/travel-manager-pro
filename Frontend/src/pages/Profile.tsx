...
import { Loader2, Lock, User, Mail, Phone, ShieldCheck, Building2, Save, MapPin, Briefcase, Camera, CalendarDays, Activity, ArrowRight, Shield } from 'lucide-react';
...
export const Profile = () => {
  ...
  const roleLabels: Record<string, { label: string; color: string; icon: any }> = {
    customer: { label: 'Khách hàng', color: 'bg-emerald-100/80 text-emerald-700 hover:bg-emerald-200 border-none ring-1 ring-emerald-200', icon: User },
    owner: { label: 'Đối tác', color: 'bg-orange-100/80 text-orange-700 hover:bg-orange-200 border-none ring-1 ring-orange-200', icon: Building2 },
    admin: { label: 'Quản trị viên', color: 'bg-blue-100/80 text-blue-700 hover:bg-blue-200 border-none ring-1 ring-blue-200', icon: ShieldCheck },
  };

  const currentRole = roleLabels[profile.role] || roleLabels.customer;
          ...
          <div className="lg:col-span-4 space-y-6 lg:space-y-8">
            <Card className="border-none shadow-2xl shadow-blue-900/5 rounded-[2rem] overflow-hidden bg-white/95 backdrop-blur-xl">
              ...
              <CardContent className="px-8 pb-8 pt-0 flex flex-col items-center text-center">
                ...
                <Badge variant="secondary" className={`mb-8 px-4 py-2 font-bold tracking-wider text-xs uppercase rounded-xl flex items-center gap-2 shadow-sm ${currentRole.color}`}>
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const profileData =
      profile.role === 'customer'
        ? { travel_style: travelStyle, date: date || undefined }
        : profile.role === 'owner'
        ...
        {profile.role === 'owner' && (
          <div className="space-y-2 md:col-span-2 group">
            <Label htmlFor="businessName" className={labelClass}>
              <Building2 size={14} className="group-focus-within:text-blue-600 transition-colors" /> Tên doanh nghiệp
            </Label>
            <Input
              id="businessName"
              value={businessName}
              ...
              className={inputClass}
              placeholder="Nhập tên thương hiệu hoặc doanh nghiệp của bạn"
            />
          </div>
        )}
        ...
        {profile.role === 'admin' && (
          <div className="space-y-2 md:col-span-2 group">
            <Label htmlFor="department" className={labelClass}>
              <Briefcase size={14} className="group-focus-within:text-blue-600 transition-colors" /> Phòng ban
            </Label>
            <Input
              id="department"
              value={department}
              ...
              className={inputClass}
              placeholder="Nhập bộ phận hoặc phòng ban công tác"
            />
          </div>
        )}
      </div>
      ...
      <DialogContent className="sm:max-w-[480px] border-none shadow-2xl shadow-blue-900/20 rounded-[2rem] p-0 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-6 bg-white">
          ...
          <div className="space-y-2">
            <Label htmlFor="current" className={labelClass}>Mật khẩu hiện tại</Label>
            <Input
              id="current"
              type="password"
